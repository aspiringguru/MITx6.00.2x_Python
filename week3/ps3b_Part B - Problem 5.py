...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        #makes a shallow copy, so it can be modified then replaced.
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances, mutProb, numTrials):
    ...
    steps = 300
    treatAtStep = 150
    prescribedDrug = ["guttagonol"]
    trialTotalPopulation = [ [] for step in range(steps)]
    trialResistantPopulation = [ [] for step in range(steps)]
    for trial in range(numTrials):
        ...
        #now progress through the timesteps
        for step in range(steps):
            if step == treatAtStep:
                patient.addPrescription('guttagonol')
            patient.update()
            trialTotalPopulation[step].append(patient.getTotalPop())
            trialResistantPopulation[step].append(patient.getResistPop(prescribedDrug))
    #reduce 2D results array to 1D, average results over trials.
    averagedTrialTotalPopulation = [sum(trials) / float(len(trials))   for trials in trialTotalPopulation]
    averagedTrialResistantPopulation = [sum(trials) / float(len(trials)) for trials in trialResistantPopulation]
    # plot results.
    pylab.plot(averagedTrialTotalPopulation, label = 'Total Virus Population')
    pylab.plot(averagedTrialResistantPopulation, label = 'Resistant Virus Population')
    pylab.title('Simulation of Virus response to prescription')
    pylab.xlabel('Time')
    pylab.ylabel('Virus Population')
    pylab.legend(loc = 'best')#standard call to position legend @ best location on plot
    pylab.show()
...
You should make use of the function
simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances, mutProb, numTrials).

As with problem 3, perform up to 100 trials and make sure that your results are repeatable and representative.

Create one plot that records both the average total virus population and the average population
of guttagonol-resistant virus particles over time.

A few good questions to consider as you look at your plots are:
What trends do you observe?
Are the trends consistent with your intuition?
Feel free to discuss the answers to these questions in the forum, to fully cement your
understanding of this problem set, processing and interpreting data.

Again, as in Problem 3, you can use the provided .pyc file to check that your
implementation of the TreatedPatient and ResistantVirus classes work as expected.
"""